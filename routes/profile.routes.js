...
router.get("/profile", isLoggedIn, isAdminOrUser, (req, res, next) => {
  User.findById(req.session.user._id)
    ...
    .then((user) => {
      ...
      console.log(
        user.dateOfBirth.toLocaleDateString("en-US", {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
        })
...
router.post("/:productId/remove", isLoggedIn, async (req, res, next) => {
  try {
    ...
    ); // filtra el carrito del usuario para eliminar el producto con el id correspondiente
...
router.get("/purchase-history", isLoggedIn, async (req, res, next) => {
  try {
    const userId = req.session.user._id;
    const user = await User.findById(userId).populate("purchasedItems");
